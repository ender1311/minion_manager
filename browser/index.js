...
const appEnter = nextRouterState => {
  Promise.all([
    axios.get('http://3.133.140.120:8000/api/minions'),
    axios.get('http://3.133.140.120:8000/api/ideas'),
    axios.get('http://3.133.140.120:8000/api/meetings'),
  ])
...
const singleMinionEnter = nextRouterState => {
  ...
  axios.get(`http://3.133.140.120:8000/api/minions/${id}`)
  .then(res => res.data)
  .then(minion => {
  ...
  axios.get(`http://3.133.140.120:8000/api/minions/${id}/work`)
  .then(res => res.data)
  .then(work => {
...
const singleIdeaEnter = nextRouterState => {
  const id = nextRouterState.params.id;
  axios.get(`http://3.133.140.120:8000/api/ideas/${id}`)
  .then(res => res.data)
  .then(idea => {
