...
const appEnter = nextRouterState => {
  Promise.all([
    axios.get('http://18.217.225.84:8080/api/minions'),
    axios.get('http://18.217.225.84:8080/api/ideas'),
    axios.get('http://18.217.225.84:8080/api/meetings'),
  ])
...
const singleMinionEnter = nextRouterState => {
  ...
  axios.get(`http://18.217.225.84:8080/api/minions/${id}`)
  .then(res => res.data)
  .then(minion => {
  ...
  axios.get(`http://18.217.225.84:8080/api/minions/${id}/work`)
  .then(res => res.data)
  .then(work => {
...
const singleIdeaEnter = nextRouterState => {
  const id = nextRouterState.params.id;
  axios.get(`http://18.217.225.84:8080/api/ideas/${id}`)
  .then(res => res.data)
  .then(idea => {
